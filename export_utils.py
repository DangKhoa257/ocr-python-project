...
def create_download_button_custom(text: str, filename: str):
    """Tạo nút tải xuống file với tên tùy chỉnh."""
    if not text or not text.strip():
        st.warning("Không có dữ liệu để tải xuống!")
        return
...
def _normalize_data(data: Union[Dict[str, Any], List[Dict[str, Any]]]) -> List[Dict[str, Any]]:
    """Chuyển đầu vào thành list các dict."""
...
def create_download_button_txt(data: Union[Dict[str, Any], List[Dict[str, Any]]], filename: str = "ket_qua_ocr.txt"):
    """Tạo nút tải TXT từ dữ liệu OCR."""
    text = format_ocr_data_to_txt(data)
    if not text.strip():
        st.warning("Không có dữ liệu để tải xuống!")
        return
    st.download_button(label=f"Tải {filename}", data=text, file_name=filename, mime="text/plain")


def create_download_button_csv(data: Union[Dict[str, Any], List[Dict[str, Any]]], filename: str = "ket_qua_ocr.csv"):
    """Tạo nút tải CSV từ dữ liệu OCR."""
    csv_data = format_ocr_data_to_csv(data)
    if not csv_data.strip():
        st.warning("Không có dữ liệu để tải xuống!")
        return
    st.download_button(label=f"Tải {filename}", data=csv_data, file_name=filename, mime="text/csv")


def create_download_button_general(data: Union[Dict[str, Any], List[Dict[str, Any]]], format_type: str = "txt", filename: str = None):
    """Tạo nút tải theo định dạng txt hoặc csv."""
    if format_type.lower() == "txt":
        create_download_button_txt(data, filename or "ket_qua_ocr.txt")
    elif format_type.lower() == "csv":
        create_download_button_csv(data, filename or "ket_qua_ocr.csv")
    else:
